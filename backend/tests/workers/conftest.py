...
def mock_image_session(mocker, mock_worker_session):
    mocker.patch("workers.image.SessionLocal", return_value=mock_worker_session)
...
def mock_embedding_models(mocker):
    ...
    mocker.patch("workers.image.get_text_embedding_model", return_value=text_model)
    mocker.patch("workers.image.get_image_embedding_model", return_value=image_model)
