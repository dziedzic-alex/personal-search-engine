...
document_processing_dead_letter_sqs_client: SQSDocumentProcessingDeadLetterClient | None = None
...
def get_document_processing_dead_letter_sqs_client() -> SQSDocumentProcessingDeadLetterClient:
    global document_processing_dead_letter_sqs_client

    if document_processing_dead_letter_sqs_client is None:
        document_processing_dead_letter_sqs_client = SQSDocumentProcessingDeadLetterClient()
...
class _SQSClient():
    def __init__(self, queue_name: str):
        self.client = boto3.resource("sqs", region_name=settings.sqs_document_queues_region).get_queue_by_name(QueueName=queue_name)
    ...
    def delete_document_message(self, receipt_handle: str) -> None:
        self.client.delete_messages(Entries=[{"Id": "1", "ReceiptHandle": receipt_handle}])
...
class SQSDocumentProcessingClient(_SQSClient):
    ...
    def submit_document_message(self, document_id: int, user_id: int) -> None:
        self.client.send_message(MessageBody=json.dumps({"document_id": document_id}), MessageGroupId=str(user_id))
