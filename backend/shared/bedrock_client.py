...
def _make_user_id_metadata_attribute(user_id: uuid.UUID) -> MetadataAttributeTypeDef:
    return {
        ...
        "value": {"type": "STRING", "stringValue": str(user_id)},
    }


class BedrockClient:
    def __init__(self):
        self.bedrock_agent_client: AgentsforBedrockClient = boto3.client(
            "bedrock-agent"
        )
    ...
    @staticmethod
    def _to_document_status(
        document_id: uuid.UUID, status: str, status_reason: str | None = None
    ) -> DocumentStatus:
        ...
        if document_status == DocumentStatus.FAILED:
            logger.error(
                f"Document {document_id} ingestion failed: {status} - {status_reason if status_reason else 'No reason provided'}"
            )

        return document_status

    def ingest_text_document(
        self, document_id: uuid.UUID, s3_content_key: str, user_id: uuid.UUID
    ) -> DocumentStatus:
            ...
            documents=[
                {
                    "metadata": {
                        ...
                        "inlineAttributes": [_make_user_id_metadata_attribute(user_id)],
                    },
                    "content": {
                        "dataSourceType": "CUSTOM",
                        "custom": {
                            "customDocumentIdentifier": {"id": str(document_id)},
    ...
    def ingest_image_document(
        ...
        if len(image_data) > self.MAX_BEDROCK_INGESTION_IMAGE_DOCUMENT_SIZE_BYTES:
            raise ValueError(
                f"Image data size {len(image_data)} bytes is greater than the maximum allowed size of {self.MAX_BEDROCK_INGESTION_IMAGE_DOCUMENT_SIZE_BYTES} bytes"
            )
    ...
    def get_documents_ingestion_statuses(
        self, document_ids: list[uuid.UUID]
    ) -> list[DocumentIngestionStatus]:
        ...
        for document_id in document_ids:
            document_identifiers_request.append(
                {"dataSourceType": "CUSTOM", "custom": {"id": str(document_id)}}
            )

        ingestion_statuses = []
        for i in range(
            0, len(document_ids), self.MAX_NUM_DOCUMENTS_PER_GET_KB_DOCUMENTS_REQUEST
        ):
            response = self.bedrock_agent_client.get_knowledge_base_documents(
                knowledgeBaseId=settings.knowledge_base_id,
                dataSourceId=settings.knowledge_base_data_source_id,
                documentIdentifiers=document_identifiers_request[
                    i : i + self.MAX_NUM_DOCUMENTS_PER_GET_KB_DOCUMENTS_REQUEST
                ],
            )
            ...
            for response_document in response_documents_details:
                document_id = (
                    response_document["identifier"].get("custom", {}).get("id")
                )
                ...
                ingestion_statuses.append(
                    self.DocumentIngestionStatus(
                        document_id=document_id,
                        status=self._to_document_status(
                            document_id, status, status_reason
                        ),
                    )
                )
    ...
    def delete_documents(self, document_ids: list[uuid.UUID]):
        ...
        for i in range(
            0, len(documents_to_delete), self.MAX_NUM_DOCUMENTS_PER_DELETE_REQUEST
        ):
            self.bedrock_agent_client.delete_knowledge_base_documents(
                knowledgeBaseId=settings.knowledge_base_id,
                dataSourceId=settings.knowledge_base_data_source_id,
                documentIdentifiers=documents_to_delete[
                    i : i + self.MAX_NUM_DOCUMENTS_PER_DELETE_REQUEST
                ],
            )
    ...
    def retrieve_relevant_document_chunks(
        self, query: str, user_id: uuid.UUID
    ) -> list[RelevantDocumentChunk]:
        ...
        for retrieval_result in retrieval_results:
            ...
            relevant_chunks.append(
                self.RelevantDocumentChunk(
                    document_id=uuid.UUID(document_id), score=float(score)
                )
            )
